...
**Defaults to 2x/Retina resolution** (`--size 44 --point-size 32`) because the Touch Bar is a Retina display — a 1x (22x22px) icon gets visibly blurred when the OS upscales it to fill the button. Only pass `--size`/`--point-size` explicitly if you want a smaller icon or need to match the old 1x output.

```sh
# From an existing image file (logo, downloaded icon, screenshot, etc.)
./scripts/icon-to-base64.sh --image ~/Downloads/some-icon.png [--size 44]

# From an SF Symbol (system icon set) with a chosen tint color
./scripts/icon-to-base64.sh --sf-symbol "wrench.and.screwdriver" --color white [--size 44] [--point-size 32]
...
./scripts/icon-to-base64.sh --emoji "🔥" [--size 44]
```
